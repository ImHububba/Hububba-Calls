...
// peers: name -> { pc, videoEl, wrap, stopVAD? }
const peers = new Map();
...
function addPeerCard(name){
  ...
  peers.set(name, { pc:null, videoEl:v, wrap, stopVAD:null });
  return v;
}
...
function cleanupAfterLeave(){
  cleanupAllPeers();
  myRoom=""; elRoomTitle.textContent="No room"; elTimer.textContent="";
...
socket.on("kicked", (info = {})=>{
  // You have been kicked—clean up and display message
